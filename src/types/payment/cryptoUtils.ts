...
export function getCryptogramRSAValue(data: CryptogramValueProps, rawPublicKey: string) {
...
function derive(pubKeyBytes: Buffer, ephemeralPrivateKey: elliptic.ec.KeyPair): Buffer {
    try {
...
async function encrypt(pubKey: string, message: string): Promise<string> {
    try {
        ...
        const ephemeralPublicKeyBytes = Buffer.from(ephemeralPrivateKey.getPublic(false, 'array'));
        ...
        const sharedKeyHash = SHA512(enc.Hex.parse(sharedSecret.toString('hex'))).toString(enc.Hex);
        ...
        const dataToMac = Buffer.concat([ivBytes, ephemeralPublicKeyBytes, cipherBytes]);
        
        const mac = HmacSHA256(enc.Hex.parse(dataToMac.toString('hex')), macKey).toString(enc.Base64);
        ...
        const finalString = Buffer.from(JSON.stringify(encryptedData)).toString('base64');
...
export async function getCryptogramECDHValue(data: CryptogramValueProps, publicKey: string): Promise<string> {
    try {
